...
<button
  ...
  onclick={handleClick}
  {disabled}
  class="google-button flex max-h-9 items-center px-3 py-1 text-xs font-medium md:px-4 md:py-2 md:text-sm [&_span]:mr-3"
>
  {#if iconName}
    <iconify-icon icon={iconName} class="mr-2 text-[18px] md:mr-3"></iconify-icon>
  {:else}
    <span>👍</span>
  {/if}
  {text}
